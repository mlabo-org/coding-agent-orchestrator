...
const META_ARGS = [
  ...
  ".CAO runner state is generated workflow state",
...
test("completion status synonyms cannot bypass gate-required completion validation", () => {
  ...
  try {
    ...
    writeFileSync(path.join(repo, ".CAO", "runner.md"), completionSynonymRunner("meta-completion-synonyms", "success"), "utf8");
...
test("normalization recovers stale pre-gate state without faking completed evidence", () => {
  ...
  try {
    ...
    writeFileSync(path.join(repo, ".CAO", "runner.md"), staleRunner("meta-stale"), "utf8");
...
test("normalization preserves audit and handoff appendices outside generated marker ownership", () => {
  ...
  try {
    ...
    for (const item of cases) {
      const filePath = path.join(repo, ".CAO", item.file);
...
test("normalization and intake strip stale active-looking preamble from generated workflow state", () => {
  ...
  try {
    ...
    for (const file of ["task.md", "project.md", "decisions.md", "handoff.md"]) {
      const filePath = path.join(repo, ".CAO", file);
...
function metacognitiveEvidenceLines() {
  const values = {
    ...
    generated_artifact_boundary: "generated runner.md packet is artifact under .CAO and fenced body is not structural",
