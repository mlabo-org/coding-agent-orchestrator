...
name: coding-agents
description: >-
  Coding Agents state-first continuation and new-task intake. Select when named, when valid .coding-agents state accompanies a concrete continuation objective, or when auditing/repairing state or CLI. Inspect state first; semantically resume related work and regenerate only clearly unrelated work.
---
...
- `{git-root}/.coding-agents/task.md`: current task SSOT, including purpose, scope, semantic `work_type`, optional typed `evidence_ref`, non-goals, completion conditions, permitted hierarchy mode, and supervision defaults when the task uses long-running or delegated assignments.
...
5. After semantic existing-state triage, run intake only when state is missing or the request is clearly unrelated. Choose a fresh `task_id`, `epoch`, and `scope`; intake replaces the current generated task documents while preserving historical `runner.md` packets. For related work, preserve completed progress and append/resume the existing lineage instead of running fresh intake:
   `node "$CODING_AGENTS_SOURCE_ROOT/bin/coding-agents.mjs" intake --target-cwd {jobsite} --work-type {auto|documentation|source-change|debug} [--evidence-ref {typed-ref}] --task {task} --task-id {id} --epoch {epoch} --scope {scope}`.
...
15. Report final status with changed files, verification, open risks, and remaining work items.
