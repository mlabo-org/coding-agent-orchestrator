...
const LEGACY_RELS = [".coding-agents", "docs/codex", "doc/codex"];
const SKIP_DIR_NAMES = new Set([
  ".git",
  ".coding-agents",
  ".CAO",
  ".CAO-migration-backups",
  ".codex",
  "node_modules",
  "vendor",
  "dist",
...
Default behavior:
  ...
  - copy plans target <git-root>/.CAO/ and never delete legacy directories
...
function discoverRepoPlans(args, inputs, audit) {
  ...
  for (const repo of Array.from(reposByRoot.values()).sort((a, b) => a.root.localeCompare(b.root))) {
    ...
    plans.push({
      ...
      targetDir: path.join(repo.root, ".CAO"),
      backupDir: "",
...
function excludePatternsFor(legacyDirs) {
  const patterns = [".CAO/", ".CAO-migration-backups/"];
  for (const legacyDir of legacyDirs) {
...
function applyPlan(plan, runId) {
  try {
    plan.backupDir = path.join(plan.repoRoot, ".CAO-migration-backups", runId);
...
function createPreflightBackup(plan) {
  ...
  if (existsSync(plan.targetDir)) {
    copyTree(plan.targetDir, path.join(plan.backupDir, "existing-dot-CAO"), {
      onSkip: (message) => plan.warnings.push(`backup skip: ${message}`),
    });
  }
...
function copyLegacyContents(context) {
  ...
  context.actions.push(
    `copied ${context.label} -> .CAO/ (new=${copied}, identical=${identical}, conflicts-preserved=${conflicts}, skipped=${skipped})`,
  );
}
...
function renderAuditReport(audit) {
  ...
  if (audit.repositories.length === 0) {
    ...
    for (const plan of audit.repositories) {
      ...
      lines.push(`- target: ${slash(path.relative(plan.repoRoot, plan.targetDir)) || ".CAO"}`);
...
function plannedActions(plan) {
  ...
  actions.push(`would copy legacy contents to .CAO/ without overwriting divergent files`);
